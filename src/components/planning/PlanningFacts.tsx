  ...
  const renderFactsContent = () => {
    switch (activeFactsTab) {
      ...
      default:
        return (
          <div>
            <h3 className="text-xl font-semibold mb-4">{factsItems.find(item => item.id === activeFactsTab)?.name}</h3>
            <p className="text-gray-600">Content for {factsItems.find(item => item.id === activeFactsTab)?.name} coming soon...</p>
          </div>
        );
    }
  };
  ...
  return (
    <div className="flex h-full">
      ...
      <div className="w-80 bg-gray-50 border-r">
        <div className="p-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">YOUR FACTS</h3>
